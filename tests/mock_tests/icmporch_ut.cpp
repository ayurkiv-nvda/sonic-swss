...
namespace icmporch_test
{
     ...
     * Create still succeeds without SAI_ICMP_ECHO_SESSION_ATTR_STATS_COUNT_MODE (SAI default).
     */
    ...
    /**
     * resolve_stats_count_mode: metadata not marked enum false; create uses SAI default.
     */
    ...
    /**
     * Capability returns success with an empty list no preferred mode; create uses SAI default.
     */
    ...
    /**
     * resolve_stats_count_mode: capability includes PACKET_AND_BYTE that mode
     * is chosen (first entry in preferred_modes) and create succeeds.
     * (Does not exercise fallback to PACKET / BYTE / NONE alone.)
     */
